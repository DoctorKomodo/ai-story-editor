...
- **`.env.example`:** `VENICE_API_KEY` has been removed everywhere (it was the legacy server-wide key; BYOK supersedes), and `APP_ENCRYPTION_KEY` is the only encryption-related env var. `CONTENT_ENCRYPTION_KEY` is never added — the boot validator (`backend/src/boot/env-validation.ts`) warns if it's accidentally re-introduced.
...
It is **in-lane for any change to**: `backend/src/services/auth.service.ts`, `backend/src/services/crypto.service.ts`, `backend/src/services/content-crypto.service.ts`, `backend/src/services/venice-call.service.ts`, `backend/src/lib/venice.ts`, `backend/src/services/session-store.ts`, `backend/src/middleware/`, `backend/src/routes/auth.routes.ts`, `backend/src/routes/venice-key.routes.ts`, or the `cookie` / `cors` / `helmet` / `rate-limit` / encryption-key bootstrap in `backend/src/index.ts`. (The historical task groups that built these surfaces — AU*, E3/E9/E12/E14, V17/V18, I7 — are closed; their detail lives in `docs/done/done-{AU,E,V,I}.md`.)
...
- BYOK Venice key: only `crypto.service` (the AES-256-GCM helper keyed by `APP_ENCRYPTION_KEY`), `venice-key.service` (store / validate / get), and `lib/venice.ts` (per-user client) touch the plaintext key, and only within the lifetime of a single request. (`content-crypto.service` is the *narrative* DEK — unrelated.) Never log it, never echo it, never serialize it to an error object
