...
- [ ] **[T8.1]** Restore the autosave + usage-indicator assertions to the [T8] full-flow spec. (1) Diagnose why the chapter PATCH never fires inside the live stack while the same flow works manually in a real browser — first hypothesis is the `EditorPage` useEffect on `[activeChapterId, chapterQuery.data]` (line 215-220 today) blowing away `draftBodyJson` when TanStack Query refetches with a new reference even though `staleTime: 30s` and `refetchOnWindowFocus: false` should prevent it. Confirm with a `page.evaluate` snapshot of the autosave hook's status during the spec; if confirmed, gate the reset on `draftBodyJson === null` rather than every change of `chapterQuery.data`. (2) Add UsageIndicator coverage by switching the AI-call site from Continue-Writing (which streams into a sibling region with no UsageIndicator) to the AIPanel `triggerAskAI` path or the chat composer Send — both render `<UsageIndicator usage={completion.usage} />` whose tail-frame totals the spec can assert against the mock's fixed `prompt_tokens=12 / completion_tokens=7 / total_tokens=19`.
  - verify: `docker compose up -d && npx playwright test tests/e2e/full-flow.spec.ts --reporter=line`
