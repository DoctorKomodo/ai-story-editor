...
- [ ] **[AU14]** (Optional, security upgrade) Swap `bcryptjs(12)` for `argon2id` (OWASP 2024 top pick). On next login, if stored hash is bcrypt, re-hash password with argon2id and update. Requires `APP_ENCRYPTION_KEY`-independent `ARGON2_PEPPER` (optional).
...
> Envelope encryption: per-user DEK (32-byte random) wrapped by a master `CONTENT_ENCRYPTION_KEY` KEK in env. All narrative content (titles, bodies, notes, character bios, outline items, chat messages) is encrypted client-of-Postgres with AES-256-GCM. Structural metadata (orderIndex, wordCount, status, FK ids, timestamps) stays plaintext so queries, ordering, and progress calcs keep working. Reuses the AES-256-GCM primitive from [AU11]. **Invoke `security-reviewer` after [E3] (key model), [E9] (repo layer), and [E12] (leak test) are implemented.**
>
> **Note — DEK is NOT derived from the user's password.** The DEK is random per user, generated at signup, wrapped by the server-side KEK. This means (a) password reset is safe (no data loss), (b) the server can decrypt user content without the user being logged in (e.g. background jobs, admin flows), and (c) a full server compromise (DB + env) defeats the encryption. Deriving the DEK from the password instead would make the encryption resistant to server-side compromise but would make password reset = data loss. **Revisit candidate** — flagged for future reconsideration once the app's threat model is clearer.

- [ ] **[E1]** Write `docs/encryption.md`: KEK / DEK model, exact field list (what's encrypted vs plaintext), threat model (what a DB dump alone reveals vs a DB + env leak), key rotation procedure, backup warning, trade-offs accepted (no DB-side FTS / title sort). **Must include a "DEK provenance" section** documenting that the DEK is random (not password-derived), the rationale (password reset safety, offline decrypt, background jobs), the weakness this accepts (DB + env compromise = data disclosure), and a "Revisit" sub-heading flagging password-derived DEKs as a future option with its own trade-offs (stronger against server compromise, incompatible with password reset as currently specced). This is the design-of-record; subsequent E-tasks reference it.
  - verify: `test -f docs/encryption.md && grep -q "CONTENT_ENCRYPTION_KEY" docs/encryption.md && grep -q "DEK" docs/encryption.md && grep -q "threat model" docs/encryption.md && grep -q "password-derived" docs/encryption.md && grep -q "Revisit" docs/encryption.md`

- [ ] **[E2]** Env + boot validation: add `CONTENT_ENCRYPTION_KEY` to `.env.example` (32-byte base64, documented separately from `APP_ENCRYPTION_KEY` — rotating one must not require re-wrapping the other). Backend startup asserts the var is set and correctly sized; fails fast with a clear, actionable message otherwise. Include a generation one-liner (`node -e "console.log(crypto.randomBytes(32).toString('base64'))"`) in `.env.example` comments.
  - verify: `grep -q "CONTENT_ENCRYPTION_KEY" .env.example && cd backend && npm run test:backend -- --run tests/boot/content-key.test.ts`

- [ ] **[E3]** Per-user DEK + content-crypto service:
  - Schema: add `User.contentDekEnc String`, `User.contentDekIv String`, `User.contentDekAuthTag String` (all non-null after backfill). For pre-existing users, migration generates a fresh DEK and wraps it with the KEK.
  - `backend/src/services/content-crypto.service.ts`: generates a DEK on signup (after [AU9] creates the user), wraps via [AU11]'s `encrypt()` with KEK, persists. Exposes `encryptForUser(userId, plaintext)` → `{ ciphertext, iv, authTag }` and `decryptForUser(userId, { ciphertext, iv, authTag })` → `plaintext`. Unwrapped DEKs live **only** in a request-scoped `WeakMap` — never a module-level cache.
...
- [ ] **[E14]** Key rotation script: `backend/prisma/scripts/rotate-kek.ts`. Reads `CONTENT_ENCRYPTION_KEY_OLD` + `CONTENT_ENCRYPTION_KEY_NEW` from env, iterates users, unwraps DEK with OLD, rewraps with NEW, writes in a transaction. **Does not touch story bytes** (that's the point of envelope encryption). Logs only counts. Documented in [E1]'s `docs/encryption.md`.
  - verify: `cd backend && npm run test:backend -- --run tests/services/kek-rotation.test.ts`

- [ ] **[E15]** SELF_HOSTING.md key-backup section (amends [I6]): documents that `CONTENT_ENCRYPTION_KEY` and `APP_ENCRYPTION_KEY` must be backed up with the same rigour as Postgres. Key loss = irrecoverable data loss. Backup recipe: write both keys to an offline location (printed or encrypted USB), test a recovery drill on a staging instance quarterly.
  - verify: `grep -q "CONTENT_ENCRYPTION_KEY" SELF_HOSTING.md && grep -q "backup" SELF_HOSTING.md && grep -q "key loss" SELF_HOSTING.md`
...
- [ ] **[V7]** Web search: add optional `enableWebSearch` boolean to `POST /api/ai/complete`. When true, set `venice_parameters.enable_web_search = "auto"` and `venice_parameters.enable_web_citations = true`. Useful for users researching facts for their story world.
  - verify: `cd backend && npm run test:backend -- --run tests/ai/web-search.test.ts`

- [ ] **[V8]** Prompt caching: set `venice_parameters.prompt_cache_key` to a deterministic hash of `storyId + modelId` on all `/api/ai/complete` requests. This improves cache hit rates by routing requests with the same story context to the same Venice backend infrastructure. Document in `docs/venice-integration.md`.
...
- [ ] **[B1]** `GET /api/stories` and `POST /api/stories`. GET returns all user stories with chapter count and total word count. POST validates with Zod.
