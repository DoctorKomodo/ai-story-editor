...
- [ ] **[F57]** Migrate scattered `keydown` listeners to `useKeyboardShortcuts` (F47): replace the document-level `keydown` blocks in `<SelectionBubble>`, `<InlineAIResult>`, `<ContinueWriting>`, `<StoryPicker>`, `<ModelPicker>`, `<Settings>`, `<CharacterPopover>` with `useEscape` / `useAltEnter` / `useModEnter` registrations. Pick priorities so an open modal's Escape closes the modal first (priority 100), then popovers (50), then the selection bubble (10). Confirm Cmd/Ctrl+Enter still submits in `<ChatComposer>` — that one stays a textarea-local listener, not document-level. Delete each component's now-redundant raw listener block.
