...
- [ ] **[E10]** Backfill migration: encrypt all existing plaintext rows for every user via the repo layer. Script: `backend/prisma/scripts/encrypt-backfill.ts`. Idempotent (safe to re-run; skips rows whose ciphertext columns are non-null). Runs inside a transaction per user. Logs counts, no content. **Invoke `repo-boundary-reviewer` after implementation** — the script is the one-off path most likely to bypass the repo layer.
