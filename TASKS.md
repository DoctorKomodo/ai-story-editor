...
- **Backlog (next):** M1–M3 maintenance, X28, X30 testing-found UI/settings polish, X32 venice-account endpoint.
...
- [ ] **[X34]** Venice call observability. Today the AI/chat routes only `console.error` on failure — there is zero log line for a successful Venice call, so manual smoke-tests can't confirm what was actually sent (action, model id, max_completion_tokens, prompt-cache key, web-search flag). Came up while testing PR #69's max_completion_tokens fix: the PR test plan said "backend log shows max_completion_tokens: 800" but no such log exists. Decide between (a) a one-line dev-only `console.log` in `ai.routes.ts` + `chat.routes.ts` right before `client.chat.completions.create({...})` printing `{ userId, modelId, action, max_completion_tokens, model_cap, user_setting, enable_web_search }` (cheap, dev-only, gated on `NODE_ENV !== 'production'`); (b) a structured logger (`pino` etc.) with a request-scoped child logger and an `ai_call` event — bigger lift, useful if we ever ship to a log-aggregation backend; (c) a debug response header (`x-debug-max-completion-tokens`, `x-debug-action`, …) that surfaces in browser DevTools without touching the log. Recommendation: do (a) now (5-line change, no design needed) and add (c) opportunistically as part of the next AI-route-touching PR. Defer (b) until we actually need structured logs. **Must NOT log** the decrypted Venice key, decrypted narrative content (chapter bodies, prompts, character bios), or the assembled `messages` array — those are governed by the leak rules in CLAUDE.md and would make this a production risk if anyone later flips the env gate.
