...
| Checksum | **None** — all 32 chars are random payload. A typo surfaces as a failed unwrap (`InvalidRecoveryCodeError`) at the reset endpoint, not client-side. The original design sketched a 16-bit CRC last group; it was never implemented, and 160 bits already exceeds the ≥128-bit floor without reserving payload for a check digit. |
...
- **2026-07-02** — [story-editor-4ow] Doc truth pass. Removed three stale claims that outlived their code: (1) the `argon2.needsRehash` rehash-on-login description (deleted by [X10] on 2026-04-24 but still described as live in the parameters section, trade-offs table, and Revisit §2) — the doc now states there is no automatic rehash and records the accepted gap; (2) the recovery-code format (a ten-group-of-sixteen-bits grouping claim and a 16-bit CRC checksum-group claim) — corrected to the implemented format: 160 bits, 4×8 Crockford chars, no checksum; (3) the claim that the frontend shows a distinct re-authenticate banner on `session_expired` — all 401s share one `onUnauthorized` path; noted as a UX gap. No code changed.
