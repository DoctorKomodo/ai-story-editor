...
- **Known residual gaps (accepted, documented):** (a) a stale draft whose server moved ahead is discarded at load (Design decisions 3) — the live version of that race is handled by Task 3's banner; (b) a keepalive flush that lands and then the tab is re-shown can produce a one-off self-conflict 409 (Task 2 Step 5 note); (c) IDB-unavailable environments degrade silently to today's behavior; (d) [post-landing, whole-branch review] summarising the actively-edited chapter bumps `updatedAt` server-side, so a pending autosave firing in the sub-second window before the summary refetch refreshes `serverUpdatedAtRef` can hit a spurious 409 — self-healing via the conflict banner, narrow enough to note rather than fix; (e) [post-landing] mid-conflict navigation discards post-conflict typing on return — tracked as story-editor-wdp.
