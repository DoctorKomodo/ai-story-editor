...
- On a hit, `req.user = { id, sessionId }` and the request-scoped DEK is attached via `attachDekToRequest(req, session.dek)`. No auth → no DEK → narrative repos throw.
